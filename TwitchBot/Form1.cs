                    ...
                    default:
                        try
                        {
                            ...
                            if (preamble[1] == "PRIVMSG")
                            {
                                ...
                                // Ignore from users/bots
                                if (sendingUser[0] != "moobot" && sendingUser[0] != "whale_bot")// && sendingUser[0] != "darabotthis")
                                {
                                    Console.WriteLine(tochat);
                                    // Chat commands
                                    if (message[2].Contains("!testbot"))
                                    {
                                        Console.WriteLine("Test Triggered by: " + sendingUser[0]);
                                        tochat = "'Beep Boop' have been triggered by user: " + sendingUser[0] + ".\n";
                                        SendKeys.SendWait(tochat);
                                    }
                                    else if (message[2].Contains("!twitter"))
                                    {
                                        Console.WriteLine("Social Links Triggered by: " + sendingUser[0]);
                                        tochat = "You can find me on the tweeters at https://twitter.com/darathiseuw \n";
                                        SendKeys.SendWait(tochat);
                                    }
                                    else if (message[2].Contains("!schedule"))
                                    {
                                        Console.WriteLine("Schedule Triggered by: " + sendingUser[0]);
                                        tochat = "LUL \n";
                                        SendKeys.SendWait(tochat);
                                    }
                                }
